...
const now = Date.now();
const sessions = [
  { id: "m1", name: "ui-polish", state: "started", status: "idle", region: "arn", created: new Date(now - 5 * 60000).toISOString(), environment: "default", repos: "https://github.com/DE0CH/claude-env.git", permissionMode: "auto", model: "claude-fable-5-1", autoPause: "on", pauseInMs: 42 * 60000, guest: "4×shared · 4 GB", aiTitle: "UI Polish" },
...
app.post("/api/sessions", (req, res) => { const id = "m" + (sessions.length + 1); sessions.push({ id, name: req.body.label || "new", state: "created", region: "arn", created: new Date().toISOString(), ...req.body, repos: (req.body.repos || []).join(" "), autoPause: req.body.autoPause === false ? "off" : "on" }); setTimeout(() => { const s = sessions.find((x) => x.id === id); s.state = "started"; s.status = "idle"; }, 4000); res.json({ ok: true, id }); });
