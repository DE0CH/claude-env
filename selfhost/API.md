...
| POST | `/api/sessions/:id/env` | `{env:{KEY:"value",…}}` → `{ok, snapshot, env:[names]}`. Inject/patch env vars into the session and restart it on the **same** conversation (`wakeMachine`): the vars are written into the machine config env so they **persist across all future restarts**, and the supervisor `--resume`s with them present. The non-hacky way to hand a running session a secret it didn't boot with (e.g. a newly added API key). **400** if `env` is empty; **409 `{needLogin:true}`** if the stored login is dead |
| POST | `/api/sessions/:id/restart` | `{env?:{K:V}, rollback?:{dropFromMarker:string}}` → `{ok, snapshot, env:[names], rolledBack}`. Restart a session, optionally patching env (as `/env`) and/or rolling its transcript back. With `rollback`, the machine is paused first (so its snapshot exists) and each snapshot transcript is truncated — every JSONL entry from the first line containing `dropFromMarker` onward is dropped — before the resume, so the session comes back **without those turns** (the clean version of hand-editing the `.jsonl`, e.g. to remove injected cross-session messages that confused it). `rolledBack` = `{machineId, transcripts:[{name,before,after,dropped}]}`. API only (no dashboard control) |
