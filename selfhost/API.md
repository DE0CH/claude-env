...
| POST | `/api/sessions/:id/stop`, `/start` | pause / wake the Fly machine. Pause first snapshots the transcript(s) + `~/.claude.json` to `claude-records/.paused/<id>/` on the Storage Box (a Fly stop resets the rootfs), then stops; **500 = snapshot failed, machine left running**. On start the session image restores the snapshot and `--resume`s the same conversation (`~/workspace` is not preserved) |
