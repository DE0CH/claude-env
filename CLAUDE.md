...
subfolder inside it named `YYYY-MM-DD <short task name>` and upload into that:
...
1. **The chat transcript from the container's disk**: the session's `.jsonl` under
   ...
   Upload the raw file (base64, `disableConversionToGoogleType: true`).
2. **Any artefacts generated by tool use**: e.g. Browserbase session recordings
   (fetch via `browse cloud sessions` recording/downloads APIs), screenshots,
   downloaded/generated files, reports.
