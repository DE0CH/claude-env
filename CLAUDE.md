...
1. **The chat transcript from the container's disk**: the session's `.jsonl` under
   ...
   Upload it as PLAIN TEXT (`textContent`, `disableConversionToGoogleType: true`).
   **NEVER use base64 (`base64Content` or the `base64` CLI) on transcripts or
   ...
   exfiltration-shaped. Don't retry it; upload text content as text.** Consequence:
   binary artefacts can't go through the connector — for those (e.g. Browserbase
   recordings), record their location/URL in the record file instead.
2. **Any artefacts generated by tool use**: e.g. Browserbase session recordings,
   screenshots, downloaded/generated files, reports. Text artefacts upload
   directly; binary ones can't (no base64 — see above), so list their
