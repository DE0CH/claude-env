...
## End-of-task records → Google Drive `/Claude Records`

At the end of EVERY task, upload the task's records to the Google Drive folder
**Claude Records** (folder ID `1lwmr6JE_51udrvKdFpHlSi090O54VWdx`, in My Drive root)
using the Google Drive connector (`mcp__Google_Drive__create_file` with
`parentId` set to that folder ID; verified working 2026-08-17). Create a per-task
subfolder inside it named `yyyy-mm-dd <session title>` — date in digits, then the
session's title (get it from `mcp__Claude_Code_Remote__get_session`, e.g.
`2026-08-17 Google Drive connector integration`) — and upload into that:
...
1. **The chat transcript from the container's disk**: the session's `.jsonl` under
   ...
   Upload it (and any large or binary file) with **`scripts/drive-browser-upload.js`**
   through a regular-context Browserbase session (verified working 2026-08-17):
   the regular context is signed in as chendeyao.uk@gmail.com and the Claude
   Records folder is shared with that account as Editor, so the browser uploads
   straight into it — byte-faithful, any size, no base64. Invocation:
   `CONNECT_URL=<session connectUrl> NODE_PATH=$(npm root -g) node
   scripts/drive-browser-upload.js <folder-or-subfolder-id> <files...>`
   (create the session per browserbase.md, fetch connectUrl with
   `browse cloud sessions get <id>` — don't retype the signing key by hand).
   Do NOT push the raw transcript through the MCP connector (inline-content only —
   a transcript-sized round-trip through context is too expensive and
   corruption-prone). `scripts/drive-upload.py` (Drive API via `GDRIVE_*` env
   vars) is an alternative if those creds ever get provisioned.
   **NEVER use base64 (`base64Content` or the `base64` CLI) on transcripts or
   ...
   exfiltration-shaped. Don't retry it.** The MCP connector is still fine for
   small text files like the record itself (`textContent`,
   `disableConversionToGoogleType: true`).
2. **Any artefacts generated by tool use**: e.g. Browserbase session recordings,
   screenshots, downloaded/generated files, reports — via
   `scripts/drive-browser-upload.js` (binary-safe). Small text artefacts may go
   through the connector. If an artefact can't be fetched/uploaded, list its
   location/URL (e.g. the Browserbase session/recording ID) in the record file.
...
Notes: folder contents are listable with `search_files` query
`parentId = '<folder-id>'` (verified). If an upload is too big for the
connector, say so in the record file and note where the artefact lives instead.
This is a standing rule — do it without being asked, before reporting the task done.
