...
1. **The chat transcript from the container's disk**: the session's `.jsonl` under
   ...
   Upload it (and any large or binary file) with **`scripts/drive-upload.py`**
   (`python3 scripts/drive-upload.py <file> --subfolder "yyyy-mm-dd <session title>"`),
   which uses the Drive API directly via the `GDRIVE_CLIENT_ID` /
   `GDRIVE_CLIENT_SECRET` / `GDRIVE_REFRESH_TOKEN` env vars — byte-faithful, any
   size, works for binaries. If those env vars are missing, do NOT try to push the
   raw transcript through the MCP connector (it only takes inline content — a
   transcript-sized round-trip through context is too expensive and corruption-prone);
   note in the record file that the transcript upload needs the creds provisioned
   (setup instructions are in the script's header).
...
2. **Any artefacts generated by tool use**: e.g. Browserbase session recordings,
   screenshots, downloaded/generated files, reports — via `scripts/drive-upload.py`
   (binary-safe). Small text artefacts may go through the connector. If an artefact
   can't be fetched/uploaded, list its location/URL (e.g. the Browserbase
