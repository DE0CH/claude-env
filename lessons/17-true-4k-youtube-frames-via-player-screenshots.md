## True-4K YouTube frames via player screenshots (2026-08, `L4Hel6VNebg`)
...
For real 4K frames, skip the download and screenshot the player in a logged-in browser
driven over CDP (claude-in-chrome on the Mac, or a mobilerun cloud phone's Chrome;
playwright is installed globally for node — `NODE_PATH=$(npm root -g)`, connect with
...
- **Viewport**: `Emulation.setDeviceMetricsOverride {width:3840, height:2160, deviceScaleFactor:1}`
  via `ctx.newCDPSession(page)` works over remote CDP on a desktop Chrome (window.innerWidth
  really becomes 3840; untested on a phone's Chrome). DSF:2 + Playwright
  `screenshot({scale:'device'})` does NOT give device pixels here — Playwright ignores
  emulation it didn't set itself. Use a big CSS viewport at DSF 1 instead.
...
- ~25s per frame end-to-end; batch everything into one CDP connection and mind 10-minute Bash
  timeouts (run in background).
