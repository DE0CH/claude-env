...
| re-login Claude | portal Settings → Re-login (drives `claude auth login --claudeai` in a PTY; paste the code). Normally unnecessary: the portal refreshes the stored OAuth pair before each session start (`ensureFreshCredentials`, `lib/oauth.js`) and every session pushes its refreshed `~/.claude/.credentials.json` back (`push-claude-credentials` → `POST /api/credentials`, newest expiry wins). Symptom of a dead login: new sessions show "Not logged in · Run /login" in their terminal and never appear in the app; `/api/state` → `creds.stale`, the dashboard shows a red banner, `POST /api/sessions` → 409 `needLogin` |
