...
| **move a running session to a bigger machine** | resize it in place — the pause snapshot is keyed by the machine id, so the same machine can come back with a new `guest`: `POST /api/sessions/:id/stop` (snapshot + stop), `flyctl machine update <id> -a de0ch-claude-sessions --vm-cpus 4 --vm-memory 4096 --skip-start`, then `POST /api/sessions/:id/start` (injects the current Claude credentials, the entrypoint restores the snapshot, the supervisor `claude --remote-control … --resume`s → same Remote Control entry with full history). Resume never auto-continues — send it a prompt (SendMessage or the app) to pick the task back up. **Per-box, NOT part of the snapshot:** the cf-tunnel + `~/tunnel-share`/`~/drop` (restart `content-server.py`+`agent.js` after a wake if you need a tunnel). The 1 GB default (`small`) OOM-kills Chromium on heavy pages — use `medium`/4 GB for anything that drives a browser. |
| a woken session shows "Not logged in" / no Remote Control | its machine env still carried a dead OAuth pair (fixed 2026-09-15: Start now injects the current one — `wakeMachine` in `server.js`). For a running machine: `POST /api/sessions/:id/relogin {"text":false}` writes the current pair in, then relaunch the host in its tmux (`tmux kill-session -t claude` + the supervisor watchdog restarts it within 20 s with `--remote-control … --resume`). `claude --debug-file /tmp/x.log` shows the bridge decision (`[bridge:repl] Reattaching to persisted bridge session …` vs `Skipping: …`). |
